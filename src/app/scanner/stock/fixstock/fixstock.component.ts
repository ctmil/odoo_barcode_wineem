...
@Component({
  selector: 'app-fixstock',
  templateUrl: './fixstock.component.html',
  styleUrls: ['./fixstock.component.scss']
})
export class FixstockComponent implements OnInit {
  ...
  public fixStock() {
    console.log(this.product.qty_available);

    if (this.product.qty_available > this.stock) {
      this.lessStock(this.product.qty_available - this.stock);
    } else {
      this.loadStock(this.stock - this.product.qty_available);
    }
  }

  public loadStock(q: number) {
